...
FROM quay.io/fedora-ostree-desktops/${SOURCE_IMAGE}:${FEDORA_VERSION} AS sway_fx
...
RUN curl -L https://copr.fedorainfracloud.org/coprs/wezfurlong/wezterm-nightly/repo/fedora-${FEDORA_VERSION}/wezfurlong-wezterm-nightly-fedora-${FEDORA_VERSION}.repo > /etc/yum.repos.d/wezterm.copr.repo;          \
    ...
    curl -L https://copr.fedorainfracloud.org/coprs/varlad/zellij/repo/fedora-${FEDORA_VERSION}/varlad-zellij-fedora-${FEDORA_VERSION}.repo                           > /etc/yum.repos.d/zellij.copr.repo;           \
